...
def run_step(opts, world_size, rank, device):
    ...
    if TRAIN:
        ...
        if opts.step > 0:
            ...
            # Load previous prototypes 增加
            if os.path.exists(f"{opts.checkpoint}/{task_name}_{opts.name}_prototypes_{opts.step - 1}.pth"):
                prototypes = torch.load(f"{opts.checkpoint}/{task_name}_{opts.name}_prototypes_{opts.step - 1}.pth")
                print("Load prototypes")
            else:
                print(
                    f"Prototype files not found in {opts.checkpoint}/{task_name}_{opts.name}_prototypes_{opts.step - 1}.pth")
        ...
        if rank == 0: # 修改
            if opts.lgkd == True:
                ...
                if opts.step > 0:
                    # Write prototypes of old classes in previous step
                    new_prototypes[:trainer.old_classes] = prototypes
                torch.save(new_prototypes, f"{opts.checkpoint}/{task_name}_{opts.name}_prototypes_{opts.step}.pth")
            print("[!] Checkpoint and prototypes saved")
        ...
        trainer = Trainer(model, None,prototypes=prototypes, device=device, opts=opts, step=opts.step,classes=tasks.get_per_task_classes(opts.dataset, opts.task, opts.step))
