...
class IncrementalSegmentationModule(nn.Module):
    ...
    def avg_weight_output(self, x):
        ...
        for i, mod in enumerate(self.cls[:-1]):
            if i == 0 and (not self.opts.two_stage):
                new_weight_bg = (self.weight_old_bg.squeeze(-1).squeeze(-1)) * (
                    mod.weight[0].unsqueeze(0).squeeze(-1).squeeze(-1))
    ...
    def init_via_weight(self):
        ...
        if self.opts.two_stage:
            ...
            cls.weight.data = new_weight.unsqueeze(-1).unsqueeze(
                -1)  # + cls.weight.data * new_weight.norm() / cls.weight.norm()
    ...
    def forward(self, x, scales=None, do_flip=False, ret_intermediate=False, only_bg=False):
        ...
        # if ((not hasattr(self, 'weight_old')) or (self.weight_old is None)) and (
        #         (not hasattr(self, 'new_classifier_weight')) or (self.new_classifier_weight is None)):
        ...
        # if (hasattr(self, "weight_old") and self.weight_old is not None or hasattr(self,
        #                                                                            "new_classifier_weight") and self.new_classifier_weight is not None) and (
        # not ret_intermediate):
        # 
        ...
        # if ((hasattr(self, 'weight_old')) and (self.weight_old is not None)) or (
        #         (hasattr(self, 'new_classifier_weight')) and (self.new_classifier_weight is not None)):
