...
class LabelGuidedKnowledgeDistillationLoss(nn.Module):
    ...
    def forward(self, new_logits, old_logits, targets):
        ...
        old_prob_novel = torch.cat([old_prob_novel, old_prob_novel.new_zeros(old_prob_novel.shape[0], new_cl)], dim=1)  # 扩展旧模型输出的维度以匹配新模型
        old_prob_novel[torch.arange(old_prob_novel.shape[0]), targets[novel_mask]] = old_prob_novel[:, 0]  # 将旧模型的背景类概率移植到新类别
        ...
        old_prob_prev = torch.cat([old_prob_prev, old_prob_prev.new_zeros(old_prob_prev.shape[0], new_cl)], dim=1)  # 扩展旧模型输出的维度以匹配新模型
