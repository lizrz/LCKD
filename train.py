...
from utils.loss import (NCA, BCESigmoid, BCEWithLogitsLossWithIgnoreIndex,
                        ...
                        TestLabelGuidedKnowledgeDistillationLoss,
...
class Trainer:
    ...
    def __init__(self, model, model_old, device, opts, trainer_state=None, classes=None, step=0, prototypes=None):
        ...
        if self.step > 0:
            if opts.unkd:
                ...
                self.lkd_loss_test = TestLabelGuidedKnowledgeDistillationLoss(alpha=opts.alpha,
                                                                     prev_kd=opts.prev_kd,
                                                                     novel_kd=opts.novel_kd)
    ...
    def train(self, cur_epoch, optim, train_loader, scheduler=None, print_int=10, logger=None):
        ...
        for cur_step, (images, labels) in enumerate(train_loader):
            ...
            if self.lkd_flag:
                if self.lgkd_flag:
                    ...
                    labels_small = F.interpolate(original_labels.unsqueeze(1).float(), scale_factor=1 / 32).long()
                    # print(f"labels_small_size:{labels_small.size()}")
                    lkd = self.lkd * self.lkd_loss(features["sem_logits_small"], features_old["sem_logits_small"],
                                                   labels_small,batch_prototypes)
            ...
            # 使用混合精度训练（AMP）的 scale_loss 方法对总损失 loss_tot 进行缩放，并进行反向传播计算梯度
            with amp.scale_loss(loss_tot, optim) as scaled_loss:
                scaled_loss.backward()

            # xxx Regularizer (EWC, RW, PI)
            if self.regularizer_flag:
                # 如果是主节点（在分布式训练中），更新正则化器
                if dist.get_rank() == 0:
                    self.regularizer.update()
                # 计算正则化损失 l_reg，并乘以正则化的重要性权重 self.reg_importance
                l_reg = self.reg_importance * self.regularizer.penalty()
                # 如果 l_reg 不为 0，使用 AMP 对正则化损失进行缩放，并进行反向传播计算梯度
                if l_reg != 0.:
        ...
        # 使用 dist.reduce 函数将所有进程的损失值汇总到目标进程（通常是主进程，dst=0）
        dist.reduce(epoch_loss, dst=0)
        dist.reduce(reg_loss, dst=0)

        # 在主进程中（dist.get_rank() == 0），计算所有进程的损失平均值
        if dist.get_rank() == 0:
        ...
        with torch.no_grad():
            for i, (images, labels) in enumerate(loader):
                ...
                if self.lkd_flag and not self.kd_need_labels:
                    if self.lgkd_flag:
                        lkd = self.lkd_loss_test(outputs, outputs_old, labels)
                    else:
                        # resize new output to remove new logits and keep only the old ones
