...
DATA_ROOT=/root/autodl-tmp/lyc/data  # Should contain subdirectory ADEChallengeData2016
...
CUDA_VISIBLE_DEVICES=${GPU} torchrun --master_port ${PORT} --nproc_per_node=${NB_GPU} run.py --date ${START_DATE} --data_root ${DATA_ROOT} --overlap --batch_size ${BATCH_SIZE} --dataset ${DATASET} --name ${NAME} --task ${TASK} --step 1 --lr 0.002 --epochs 60 --method ${METHOD} --opt_level O1 ${OPTIONS} --prev_kd ${PREV_KD} --novel_kd ${NOVEL_KD} --step_ckpt ${FIRSTMODEL}
...
CUDA_VISIBLE_DEVICES=${GPU} torchrun --master_port ${PORT} --nproc_per_node=${NB_GPU} run.py --date ${START_DATE} --data_root ${DATA_ROOT} --overlap --batch_size ${BATCH_SIZE} --dataset ${DATASET} --name ${NAME} --task ${TASK} --step 4 --lr 0.002 --epochs 45 --method ${METHOD} --opt_level O1 ${OPTIONS} --prev_kd ${PREV_KD} --novel_kd ${NOVEL_KD}
